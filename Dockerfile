FROM ubuntu:jammy AS elraro-ubuntu-jammy
...
RUN set -x && \
    ...
    apt-get install -y --no-install-recommends \
            ...
            && \
    apt-get update && \
    curl -ksSLo /usr/local/bin/MailHog https://github.com/mailhog/MailHog/releases/download/v1.0.0/MailHog_linux_arm64 && \
    curl -ksSLo /usr/local/bin/mhsendmail https://github.com/mailhog/mhsendmail/releases/download/v0.2.0/mhsendmail_linux_arm64 && \
...
FROM elraro-ubuntu-jammy AS elraro-nodejs-20-ubuntu-jammy
...
### Files add
ADD freepbx-17/install /

#S6 installation
ARG TARGETARCH
RUN set -x && \
    S6_ARCH=$(echo "$TARGETARCH" | sed 's/arm64/aarch64/') && \
    curl -ksSL https://github.com/just-containers/s6-overlay/releases/download/v${S6_OVERLAY_VERSION}/s6-overlay-noarch.tar.xz | tar xJf - -C / && \
    curl -ksSL https://github.com/just-containers/s6-overlay/releases/download/v${S6_OVERLAY_VERSION}/s6-overlay-${S6_ARCH}.tar.xz | tar xJf - -C / && \
